...
function buildLadder(settings){
  ...
  for(let step=1; step<=settings.maxSteps; step++){
    if(step > 1){
      const previousLoss = cumulative - bet;
      const targetPerNumber = Number(settings.targetNum) || 0;
      const stillHitsTarget = ((bet * 8) - previousLoss) >= targetPerNumber;

      if(!stillHitsTarget){
        if((settings.doubleLadder || 'off') === 'on'){
